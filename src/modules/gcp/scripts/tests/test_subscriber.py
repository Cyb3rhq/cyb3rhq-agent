...
from pubsub.subscriber import Cyb3rhqGCloudSubscriber
...
def get_wodle_config(credentials_file: str = "credentials.json", project: str = "test_project",
    ...
    """Return a dict containing every parameter supported by Cyb3rhqGCloudSubscriber. Used to simulate different
...
def test_Cyb3rhqGCloudSubscriber__init__(mock_client):
    """Test if an instance of Cyb3rhqGCloudSubscriber is created properly."""
    pubsub = Cyb3rhqGCloudSubscriber(**get_wodle_config())
...
def test_Cyb3rhqGCloudSubscriber__init__ko(credentials_file, errcode):
    """Test that the appropriate exceptions are raised when the Cyb3rhqGCloudSubscriber constructor is called with
    invalid parameters.
    """
    with pytest.raises(GCloudError) as e:
        Cyb3rhqGCloudSubscriber(**get_wodle_config(credentials_file=os.path.join(data_path, credentials_file)))
    assert e.value.errcode == errcode


@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Cyb3rhqGCloudSubscriber_get_subscriber_client(mock_credentials):
    """Test get_subscriber_client attempts to create a client object using the provided credentials file."""
    Cyb3rhqGCloudSubscriber.get_subscriber_client("credentials.json")
    mock_credentials.assert_called_with("credentials.json")


@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Cyb3rhqGCloudSubscriber_get_subscription_path(mock_credentials):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Cyb3rhqGCloudSubscriber_check_permissions(mock_credentials):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Cyb3rhqGCloudSubscriber_check_permissions_ko(mock_credentials, errcode, msg):
    """Test check_permissions raises the expected GCloudError when the subscriber does not have the required
    permissions."""
    pubsub = Cyb3rhqGCloudSubscriber(**get_wodle_config(credentials_file="credentials"))
    if errcode != 1206:
...
@patch('pubsub.subscriber.Cyb3rhqGCloudSubscriber.send_msg')
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Cyb3rhqGCloudSubscriber_pull_request(mock_credentials, mock_send_msg, num_messages):
...
@patch('pubsub.subscriber.Cyb3rhqGCloudSubscriber.send_msg')
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Cyb3rhqGCloudSubscriber_pull_request_ko(mock_credentials, mock_send_msg):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Cyb3rhqGCloudSubscriber_process_messages(mock_credentials):
