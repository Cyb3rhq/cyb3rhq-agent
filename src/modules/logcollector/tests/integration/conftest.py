...
from cyb3rhq_testing.constants.paths.configurations import CYB3RHQ_CONF_PATH
from cyb3rhq_testing.constants.daemons import LOGCOLLECTOR_DAEMON
from cyb3rhq_testing.logger import logger
from cyb3rhq_testing.modules.logcollector.patterns import LOGCOLLECTOR_MODULE_START
...
from cyb3rhq_testing.utils.file import truncate_file, replace_regex_in_file, write_json_file

# Logcollector internal paths
LOGCOLLECTOR_OFE_PATH = path_join(CYB3RHQ_PATH, 'queue', 'logcollector', 'file_status.json')
...
def stop_logcollector(request):
    """Stop cyb3rhq-logcollector and truncate logs file."""
...
def wait_for_logcollector_start(request):
    ...
    log_monitor = FileMonitor(CYB3RHQ_LOG_PATH)
...
@pytest.fixture()
def remove_all_localfiles_cyb3rhq_config(request):
    """Configure a custom settting for testing. Restart Cyb3rhq is needed for applying the configuration. """
    # Backup the original configuration
    backup_config = configuration.get_cyb3rhq_conf()

    # Remove localfiles from the configuration
    list_tags = [r"<localfile>[\s\S]*?<\/localfile>"]
    replace_regex_in_file(list_tags, [''] * len(list_tags), CYB3RHQ_CONF_PATH, True)
