...
from cyb3rhq_testing.utils.file import write_json_file
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                reset_ofe_status, remove_all_localfiles_cyb3rhq_config, set_cyb3rhq_configuration, pre_send_journal_logs,
                                daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon starts properly when the 'journald' tag is used 
    ...
    assertions:
        ...
        - Verify that the Cyb3rhq component (agent or manager) can read the logs from the 'journald' log format and the logs older than the current logcollector start time.
