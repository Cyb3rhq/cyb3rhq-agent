...
from cyb3rhq_testing.modules.agentd import configuration as agentd_configuration
...
# Configuration
default_config_path = Path(CONFIGURATIONS_PATH, 'cyb3rhq_basic_configuration_log_format_location.yaml')
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                set_cyb3rhq_configuration, daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon starts properly when the 'location' tag is used.
                 ...
                 Cyb3rhq component is started by checking its process, and the Cyb3rhq API returns the same
    ...
    assertions:
        - Verify that the Cyb3rhq component (agent or manager) can start when the 'location' tag is used.
        - Verify that the Cyb3rhq API returns the same value for the 'localfile' section as the configured one.
    ...
    if sys.platform == WINDOWS:
        if test_metadata['location'] == 'invalidchannel':
            ...
            cyb3rhq_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT, callback=callback)
            assert (cyb3rhq_log_monitor.callback_result != None), patterns.ERROR_EVENTCHANNEL
    else:
