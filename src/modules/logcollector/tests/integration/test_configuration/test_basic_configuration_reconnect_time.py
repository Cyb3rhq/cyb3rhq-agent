...
config_path = Path(CONFIGURATIONS_PATH, 'cyb3rhq_basic_configuration_reconnect_time.yaml')
...
def test_configuration_reconnect_time(test_configuration, test_metadata, truncate_monitored_files,
    ...
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon detects invalid settings for the 'reconnect_time' tag.
    ...
    input_description: A configuration template (test_basic_configuration_reconnect_time) is contained in an
                       external YAML file (cyb3rhq_basic_configuration.yaml). That template is combined with
    ...
    if test_metadata['valid_value']:
        ...
        if test_metadata['reconnect_time'] in problematic_values:
            pytest.xfail("Logcolector accepts invalid values. Issue: https://github.com/cyb3rhq/cyb3rhq/issues/8158")
        else:
            control_service('start', daemon=LOGCOLLECTOR_DAEMON)
            ...
            assert (cyb3rhq_log_monitor.callback_result != None), patterns.ERROR_INVALID_RECONNECTION_TIME
