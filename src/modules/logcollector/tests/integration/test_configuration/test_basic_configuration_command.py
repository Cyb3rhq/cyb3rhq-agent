...
brief: The 'cyb3rhq-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       correctly executed by the logcollector, and the Cyb3rhq API returns the same values for
...
config_path = Path(CONFIGURATIONS_PATH, 'cyb3rhq_basic_configuration_command.yaml')
...
def test_configuration_command(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon can monitor commands that use multiple parameters.
    ...
    cyb3rhq_log_monitor.start(callback=callback)
