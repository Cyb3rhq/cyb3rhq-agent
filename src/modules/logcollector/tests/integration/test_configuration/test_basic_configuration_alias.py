...
brief: The 'cyb3rhq-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       specified in the 'alias' tag when monitoring a command, and the Cyb3rhq API returns the same
...
config_path = Path(CONFIGURATIONS_PATH, 'cyb3rhq_basic_configuration.yaml')
...
def test_configuration_alias(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                            set_cyb3rhq_configuration, daemons_handler):
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon changes a command name in the log messages by
                 ...
                 will verify that the Cyb3rhq API returns the same values for the 'localfile' section that
                 the configured one.
    ...
    input_description: A configuration template (test_basic_configuration_alias) is contained in an external YAML file
                       (cyb3rhq_basic_configuration.yaml). That template is combined with two test cases defined
    ...
    cyb3rhq_log_monitor.start(callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_READING_COMMAND_ALIAS, {
