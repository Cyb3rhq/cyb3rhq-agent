...
brief: The 'cyb3rhq-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'label' tag is set in the configuration, and the Cyb3rhq API returns the same values for
...
config_path = Path(CONFIGURATIONS_PATH, 'cyb3rhq_basic_configuration_label.yaml')
...
def test_configuration_label(test_configuration, test_metadata, set_cyb3rhq_configuration, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon can monitor log files configured to use labels.
                 ...
                 Finally, the test will verify that the Cyb3rhq API returns the same values for
    ...
    cyb3rhq_log_monitor.start(callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_ANALYZING_FILE, {
