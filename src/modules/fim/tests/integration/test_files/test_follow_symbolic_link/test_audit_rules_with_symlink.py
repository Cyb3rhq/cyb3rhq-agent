...
from cyb3rhq_testing.modules.fim.patterns import AUDIT_RULES_RELOADED, EVENT_TYPE_MODIFIED, LINKS_SCAN_FINALIZED
from cyb3rhq_testing.modules.monitord.configuration import MONITORD_ROTATE_LOG
from cyb3rhq_testing.modules.fim.configuration import SYMLINK_SCAN_INTERVAL, SYSCHECK_DEBUG
from cyb3rhq_testing.tools.monitors.file_monitor import FileMonitor
from cyb3rhq_testing.utils import file, commands
...
def test_audit_rules_with_symlink(test_configuration, test_metadata, set_cyb3rhq_configuration, truncate_monitored_files,
                                  configure_local_internal_options, folder_to_monitor, symlink, symlink_new_target,
                                  daemons_handler, start_monitoring):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon removes the 'audit' rules when the target of
    ...
    input_description: The test cases are contained in external YAML file (cases_delete_hardlink_symlink.yaml)
                       which includes configuration parameters for the 'cyb3rhq-syscheckd' daemon and testing
    ...
    cyb3rhq_log_monitor.start(generate_callback(LINKS_SCAN_FINALIZED))
    assert cyb3rhq_log_monitor.callback_result
    cyb3rhq_log_monitor.start(generate_callback(AUDIT_RULES_RELOADED))

    cyb3rhq_log_monitor.start(generate_callback(EVENT_TYPE_MODIFIED))
