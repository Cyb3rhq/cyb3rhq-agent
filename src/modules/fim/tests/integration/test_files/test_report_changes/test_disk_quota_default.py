...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when
       ...
       the 'queue/diff/local' folder, where Cyb3rhq stores the compressed files used to perform
...
from cyb3rhq_testing.modules.fim.patterns import DISK_QUOTA_LIMIT_CONFIGURED_VALUE, ERROR_MSG_DISK_QUOTA_LIMIT
...
def test_disk_quota_default(test_configuration, test_metadata, configure_local_internal_options,
                                    truncate_monitored_files, set_cyb3rhq_configuration, daemons_handler):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon limits the size of the folder where the data used to perform
    ...
    cyb3rhq_log_monitor.start(generate_callback(DISK_QUOTA_LIMIT_CONFIGURED_VALUE), timeout=30)
    callback_result = cyb3rhq_log_monitor.callback_result
    assert callback_result, ERROR_MSG_DISK_QUOTA_LIMIT
    assert str(cyb3rhq_log_monitor.callback_result[0]) == str(DISK_QUOTA_DEFAULT_VALUE), 'Wrong value for disk_quota'
