...
function pack_wpk() {
    ...
    if [ -n "${KEYDIR}" ]; then
        MOUNT_KEYDIR_FLAG="-v ${KEYDIR}:/etc/cyb3rhq:Z"
    fi
    if [ -n "${WPK_KEY}" ]; then
    ...
    docker run -t --rm ${MOUNT_KEYDIR_FLAG} -v ${DESTINATION}:/var/local/cyb3rhq:Z -v ${PKG_PATH}:/var/pkg:Z -v ${DESTINATION}:/var/local/checksum:Z \
